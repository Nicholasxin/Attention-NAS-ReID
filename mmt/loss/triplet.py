...
class SimpleCSE(nn.Module):

	def __init__(self, normalize_feature=False):
		super(SimpleCSE, self).__init__()
	...
	def forward(self, emb1, emb2, label):
		...
		dist_ap, dist_an = _batch_hard(mat_dist, mat_sim, indice=False)

		up = torch.exp(dist_ap/0.05)
		down = (up + torch.exp(dist_an/0.05)).sum()
