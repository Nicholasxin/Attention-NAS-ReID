...
def extract_features(model, data_loader, print_freq=200, metric=None):
    ...
    features_bn = OrderedDict()
    features_ln = OrderedDict()
    labels = OrderedDict()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _) in enumerate(data_loader):
            ...
            outputs_bn = extract_cnn_feature(model, imgs)
            for fname, outputs_bn, pid in zip(fnames, outputs_bn, pids):
                features_bn[fname] = outputs_bn
                labels[fname] = pid
